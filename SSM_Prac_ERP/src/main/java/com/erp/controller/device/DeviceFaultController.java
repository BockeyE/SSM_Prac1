...
//        session.setAttribute("sysPermissionList", arr);
    ...
    @RequestMapping("/list")
    @ResponseBody
    public ViewResult list(Integer page, Integer rows, DeviceFault vo) {
    ...
    @ResponseBody
    private OperationResult insert(@Valid DeviceFault a, BindingResult bindingResult) {
        ...
        if (ser.get(a.getDeviceFaultId()) != null) {
    ...
    @ResponseBody
    private OperationResult update(@Valid DeviceFault a, BindingResult bindingResult) throws Exception {
        if(bindingResult.hasErrors()){
            FieldError fieldError = bindingResult.getFieldError();
            return OperationResult.build(100, fieldError.getDefaultMessage());
        }
    ...
    @ResponseBody
    private OperationResult updateAll(@Valid DeviceFault a, BindingResult bindingResult) throws Exception {
        if(bindingResult.hasErrors()){
            FieldError fieldError = bindingResult.getFieldError();
            return OperationResult.build(100, fieldError.getDefaultMessage());
        }
